...
- `Assert.True(result.IsSuccess)` — xUnit assert guard
- `Assert.That(result.IsSuccess, Is.True)` — NUnit assert guard
- `result.IsSuccess.Should().BeTrue()` — FluentAssertions guard
